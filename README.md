...
`Тр` (труба) или `МР` (металлорукав), затем `Dn<диаметр>`, тире и
длина в метрах (например `Тр Dn20-15`; в одной ячейке может быть
несколько записей). Длины суммируются по типу и диаметру, и вторая
