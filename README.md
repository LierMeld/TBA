...
- Таблица кабелей: **Напряжение | Тип кабеля | Кол-во позиций |
  Экран | Суммарная длина, м**. Напряжение берётся из столбца **O**
  листа «Таблица кабелей»; «Экран» определяется по букве **Э** в
  названии типа. После таблицы — пустая строка, строка «Итого» и
  отдельная строка «в том числе экранированных».
- Таблица труб и металлорукавов: **наименование** (ячейки объединены
  до столбца длины) и **суммарная длина** в том же столбце, что и у
  кабелей. Итоговой строки нет.
- Обе таблицы имеют редактируемый заголовок над таблицей — константы
  `SPEC_CAB_TITLE` и `SPEC_PIPE_TITLE` в начале модуля.
- Форматирование: границы всех ячеек, выравнивание по центру (кроме
  наименований — по левому краю), перенос строк в шапке, шрифт и
  размер задаются константами `SPEC_FONT_NAME` / `SPEC_FONT_SIZE` /
  `SPEC_TITLE_SIZE`.
- Обе таблицы начинаются с 3-й строки от границы своего листа;
  таблица труб — всегда с нового листа.
- Лист «Спецификация» формируется заново при каждом запуске и
  оформляется рамками формата **A4 книжная** с основной надписью
  формы 6 и левым штампом на каждом листе.
- **Порядок запуска** для сквозной нумерации листов:
  **BuildSpecification → PrintWithFrame → PrintWithFrameAnn** —
  нумерация начинается с листов спецификации, затем идут листы
  таблицы кабелей, затем аннотация.
